...
export default ({ route }) => {
  ...
  const [imagenVisor, setImagenVisor] = useState<
    [
      {
        url: string;
        props?: {
          ...
          return {
            ...
            show:
              e.body !== null
                ? i <= 2
                  ? true
                  : idx === rand1 || idx === rand2 || idx === rand3
                  ? true
                  : false
                : false,

            // Obtener el usuario de cada comentario
            owner: await DataStore.query(Usuario, e.creatorID).then(
              async (r) => ({
                ...r,
  ...
  function getDateFromNow(date: Date | number) {
    if (!date) return null;
    const fecha = new Date(date);
    const msFromNow = new Date().getTime() - fecha.getTime();
  ...
  async function fetchAventurasGuia(fechas) {
    ...
    listaAventuras = await Promise.all(
      listaAventuras.map(async (e) => {
        return await DataStore.query(Aventura, e).then(async (r) => {
          return {
            id: r.id,
            titulo: r.titulo,
            imagenFondo: {
  ...
  function openImage(img: string, require?: boolean) {
    setModalVisible(true);
      ...
      <View style={styles.body}>
        <View style={styles.innerContainer}>
          ...
          >
            ...
            <Pressable
              onPress={
                editing ? handleChangePic : () => openImage(user.foto?.uri)
              }
              style={{ marginBottom: 10 }}
            >
              {user?.foto?.uri ? (
                <Image source={{ uri: user.foto.uri }} style={styles.image} />
              ) : (
                <Feather
                  style={{
            ...
            {!editing && (
              <View style={styles.headerTextContainer}>
                ...
                <View style={styles.calificacionesCont}>
                  ...
                  >
                    ({user?.numResenas ? user.numResenas : 0})
                  </Text>
                </View>
              </View>
            )}
          ...
          <View style={styles.cuadrosContainer}>
            ...
            <Pressable
              ...
              style={styles.cuadro}
            >
              <Text style={styles.titleCuadro}>
                {calculateLvl(user?.experience).lvl}
              </Text>
              {isOwner && (
              ...
              {!comentarios ? (
                ...
                <Pressable onPress={handleOpenComments}>
                  {comentarios.map((e, index) => {
                    ...
                    if (e.show) return <CommentItem key={index} content={e} />;
                  })}
                </Pressable>
              ) : (
            ...
            <View style={styles.item}>
              ...
              >
                ...
                <MaterialIcons
                  name={
                    ("keyboard-arrow-" +
                      (showExperiencias ? "up" : "down")) as any
                  }
                  size={30}
                  color={moradoClaro}
                />
              ...
              {showExperiencias &&
                (experiencias.length !== 0 ? (
                  <View style={{ marginTop: 20 }}>
                    <ExperienciasGuia experiencias={experiencias} />
                  </View>
                ) : (
                  <Text style={[styles.NoHay, { marginTop: 20 }]}>
                    No hay experiencias
                  </Text>
                ))}
            </View>
        ...
        {isOwner && (
          <Pressable
            onPress={
              loading
                ? null
                : () => (editing ? handleSaveProfile() : setEditing(!editing))
            }
            style={styles.backIcon}
          >
            {loading ? (
              <ActivityIndicator color={moradoClaro} size={"small"} />
            ) : editing ? (
              <Feather name="check" size={30} color={moradoClaro} />
            ) : (
              <Feather name="edit-2" size={25} color={moradoClaro} />
            )}
          </Pressable>
        )}
      </View>
